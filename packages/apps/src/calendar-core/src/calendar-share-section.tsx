...
export function CalendarShareSection({
  ...
  const grants = useMemo(() => calendarShareGrantEntries(calendar.shareWith), [calendar.shareWith]);
  ...
  return (
    <>
      {!online ? <p className="share-dialog__error">{labels.shareCalendarOffline}</p> : null}
      <ShareAccessCard
