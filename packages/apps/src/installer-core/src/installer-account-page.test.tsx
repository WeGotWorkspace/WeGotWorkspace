...
} from "@/installer-core/src/installer-account-page";
...
describe("InstallerAccountPage", () => {
  ...
  it("collapses username feedback until an invalid slug is shown", () => {
    vi.useFakeTimers();
    const { container } = render(<InstallerAccountPage initialUsername="jane" />);
    const usernameField = container.querySelectorAll(".installer__field")[0];
    const feedback = usernameField?.querySelector(".installer__field-feedback");
    expect(feedback).toBeTruthy();
    expect(feedback?.classList.contains("installer__field-feedback--hidden")).toBe(true);
    ...
    expect(feedback?.classList.contains("installer__field-feedback--hidden")).toBe(false);
    expect(feedback?.getAttribute("aria-live")).toBe("polite");
    expect(usernameField?.querySelectorAll(".installer__field-feedback")).toHaveLength(1);
  });
  ...
  it("shows a taken username in the username feedback slot immediately", () => {
    const { container } = render(<InstallerAccountPage initialUsername="jane" usernameTaken />);
    const feedback = container.querySelector(".installer__field-feedback");
  ...
  it("disables Create workspace until the email is valid without waiting for feedback", () => {
    vi.useFakeTimers();
    render(<InstallerAccountPage initialUsername="jane" initialPassword="hunter2hunter" />);
  ...
  it("replaces the password hint with a debounced error while it is too short", () => {
    ...
    expect(screen.getByText("At least 10 characters.")).toBeTruthy();
    expect(
      screen.getByText("At least 10 characters.").classList.contains("installer__hint--error"),
    ).toBe(false);
  ...
  it("keeps Database in the progress dots after that step", () => {
    render(<InstallerAccountPage initialUsername="jane" />);
  ...
  it("omits the Database dot when env already supplied the database", () => {
    render(<InstallerAccountPage initialUsername="jane" includeDatabaseStep={false} />);
    const progress = screen.getByRole("list", { name: "Setup progress" });
  ...
  it("shows a single install step with a spinner marker while installing", () => {
    ...
    const markers = container.querySelectorAll(".installer__progress-marker");
    expect(markers).toHaveLength(1);
    expect(markers[0]?.querySelector(".installer__spinner")).toBeTruthy();
  });
});
