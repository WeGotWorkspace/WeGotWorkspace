...
export const Welcome: Story = {
  tags: ["vitest-ci"],
  render: () => <InstallerWelcomePage />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("heading", { name: installerCopy.welcomeTitle }),
    ).toBeInTheDocument();
    await expect(canvas.getByText(installerCopy.welcomeLead)).toBeInTheDocument();
...
export const Database: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const panels = canvasElement.querySelectorAll(".installer__engine-panel");
    await expect(panels).toHaveLength(2);
    await expect(panels[1]).toHaveClass("installer__engine-panel--hidden");
...
export const DatabaseSqlite: Story = {
  ...
  tags: ["vitest-ci"],
  render: () => <InstallerDatabasePage initialEngine="sqlite" />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
export const Account: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("heading", { name: installerCopy.accountTitle }),
    ).toBeInTheDocument();
    await expect(progressItems(canvas)).toHaveLength(4);
...
export const Ready: Story = {
  tags: ["vitest-ci"],
  render: () => <InstallerReadyPage />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("heading", { name: installerCopy.readyTitle }),
    ).toBeInTheDocument();
    await expect(canvas.getByText(installerCopy.readyLead)).toBeInTheDocument();
...
export const ServerNeedsAttention: Story = {
  ...
  tags: ["vitest-ci"],
  render: () => <InstallerAttentionPage checks={FAILED_CHECKS} />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("heading", { name: installerCopy.serverTitle }),
    ).toBeInTheDocument();
