...
| **Notes** (VJOURNAL) | Vendor envelope `urn:wgw:jmap:notes` (`Notebook`/`Note` get\|changes\|set); JMAP-shaped REST `/notes/*` | Not an IETF Notes datatype; no Push | [#671](https://github.com/WeGotWorkspace/wegotworkspace/issues/671) |
...
- **Contacts are behind the envelope** (#437): `AddressBook/get|changes|set` and `ContactCard/get|changes|set|query|queryChanges` per RFC 9610, with the legacy REST shapes normalized at the adapter layer (REST untouched). **Files too** (#450, draft-ietf-jmap-filenode-14 pinned): `FileNode/get|changes|set|copy|query|queryChanges` over a node-identity index (`jmap_file_nodes`) maintained from both drive write paths. **Notes** use a **vendor** envelope (`urn:wgw:jmap:notes`, #671) — not IETF `urn:ietf:params:jmap:notes`. Tasks are **not** — the tasks envelope is explicitly out of scope (`draft-ietf-jmap-tasks-06` is an expired, immature draft). Remaining roadmap (phased mail RFC 8621, gated on the IMAP CI fixture #451): [.agents/specs/000-jmap-envelope-multidomain/](../../../.agents/specs/000-jmap-envelope-multidomain/spec.md).
...
| Area | Status | Notes |
|------|--------|-------|
| ICS/vCard payload bounds | Done ([#162](https://github.com/WeGotWorkspace/wegotworkspace/issues/162)) | Per-object 512 KiB; 64 combined nested VEVENT+VTODO+VALARM (VTIMEZONE excluded); 512 vCard properties. Contact import body default 8 MiB (`min(8 MiB, post_max_size)`); calendar import body stays 512 KiB ([#931](https://github.com/WeGotWorkspace/wegotworkspace/issues/931) for `MAX_ICS_IMPORT_BYTES` + visible over-cap signal). Per-item import `errors[].code`; JMAP set → `tooLarge` via stable codes `payload_too_large` / `payload_too_complex`; get isolates over-cap id as `notFound`; query and `GET /tasks/items` omit over-cap objects |
