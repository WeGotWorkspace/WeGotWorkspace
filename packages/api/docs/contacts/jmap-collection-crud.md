...
|------------|-------------|---------------------|
| **Calendars** | Persisted for personal owners via Sabre `CalPDO::updateInvites` / `getInvites`. JMAP ids are `username` or `groups/{slug}`. `share_href` is `mailto:` (profile email, else `mailto:{username}` / `mailto:groups/{slug}`). `mayWriteAll` (or REST `mayWrite`) → access 3; otherwise access 2. Null grant revokes. Recipients see the shared instance on their principal with `shareWith: null`. Event create/update/delete is `403 forbidden` when instance `access === 2`. Group calendars stay membership-only (shareWith writes rejected). CalDAV `CS:share` / `DAV:share-resource` and `{CS:}invite` / `{DAV:}invite` use the same rows (href ↔ JMAP id via `mailto:`). | `true` for personal owned instances (`access === 1`) |
