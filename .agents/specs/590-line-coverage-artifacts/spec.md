...
- Vitest unit and jsdom only. Each jsdom child gets `--coverage --reporter=blob --outputFile=.vitest-reports/blob-<shard>.json` and its own `--coverage.reportsDirectory`.
