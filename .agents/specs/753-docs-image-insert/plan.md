...
- **Done when:** slash + toolbar open picker or upload; paste/drop image files upload then `setImage`; custom Image attrs round-trip markdown images whose src is `drive:fn-…` in markdown + Yjs; node view resolves via authenticated download to `blob:`; external `https://` images still work; `.txt` docs stay image-free; track-changes/export does not embed bytes
