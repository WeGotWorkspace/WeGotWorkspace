...
- **Done when:** `calendar-search.ts` expands via `occurrencesInRange(..., calendarBootstrapWindow(), { visibleCalendarIds })`, then trim + exact substring on title/location/master description; split/sort/cap 100+100; `truncatedUpcoming` / `truncatedPast`. Vitest covers empty/whitespace, trim, case, exact-substring vs token order, field isolation, bootstrap-window expansion, recurrence multiples, upcoming-vs-past split/sort, per-section cap flags.
...
- **Done when:** ViewHeader flush-on-empty (non-empty → empty only); 180ms typing; no mount empty callback; pending timer cancelled; empty flush independent of `searchDebounceMs`. Calendar search chrome, keyboard shortcuts, locked period chrome, dedicated results list, CollectionState no-match + range caption, truncation captions, BEM+`@apply`. Stories + fake-timer play. Mail/Notes/Drive/Docs/Contacts search tests still green. Header-layout CSS tests kept.
...
- [ ] Mock-tier Storybook play (idle, matches, no-match, truncated, result-open, typing burst, X restore)
