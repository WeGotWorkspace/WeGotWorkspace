...
- Chrome: reuse suite `ViewHeader` + `CollectionSearchInput` (`layout="responsive"`). ≤1159px the same field is a bottom-right FAB; `--expanded` (focus) animates `width` from `w-9` to `calc(100% - start/end gutter)`. Outside tap hits a transparent `.calendar-search-dismiss` catcher (`pointerdown`/`click` swallowed, then blur) so the grid does not receive the tap; results stay if `q` is active. Desktop header field is unchanged. Wire `useWorkspaceListKeyboardShortcuts` for ⌘/Ctrl+K and `/`.
