...
| **Counted source file size** | `packages/apps/src`, `tools/mcp-server/src`, and `packages/api/app` stay at or under 400 lines. Tests, stories, declarations, mocks, fixtures, and `test-utils` are excluded. A new counted source file over 400 lines is a merge block unless its baseline entry carries an approved reason. A baselined file is a merge block when its line count grows, or when it shrinks and the stored integer was not lowered. | Apps done gate, API done gate, `mcp-server` CI job, `pnpm check:file-size`. `check:agent-docs` checks this sentence. | [clean-code/smells.md](skills/clean-code/smells.md) |
