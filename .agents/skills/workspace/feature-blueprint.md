...
- [ ] Storybook: story file per UI component (`Features/<Product>/Components/*` + `Panes/*`), Controls on simple props, named stories per meaningful state; no Storybook `decorators` (use `*-story-scope.tsx` in `render` when tokens need a workspace root); workspace chrome Default under `Branding/<Product>`; unique state matrices under `Features/<Product>`; bootstrap returns coherent **`data` + `session`** (and **`operations`** when needed).
