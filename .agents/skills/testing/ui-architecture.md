...
- Run: `pnpm test` or `pnpm test:watch` in `packages/apps` (unit + jsdom projects). `pnpm test` runs unit and sharded jsdom independently (jsdom still runs if unit fails); `test:watch` stays unsharded (interactive subset).
- Config: `packages/apps/vitest.config.ts` — **`unit`** (Node, `*.test.ts`) and **`jsdom`** (RTL, `*.test.tsx`, `pool: "forks"` + `maxWorkers: 1`). jsdom is launched by `scripts/run-jsdom.mjs`: rotate-per-package shards (package `p` starts at shard `p % N`), default `JSDOM_SHARDS=24`, plus a solo process per heap-heavy RTL file (`use-calendar-controller*`, `use-contacts-*`, `contacts-detail-view`, `workspace-live-app-shell`). In CI the packed pool and the solo pool run with the caps in `scripts/jsdom-concurrency.mjs` (3 and 2 on a 4-core / 16 GB GitHub runner). Locally both stay at 1. `isolate: true` does not reclaim jsdom heap — do not raise `--max-old-space-size` or the pool width for the next OOM. A single huge RTL file can still OOM a shard — split that file or keep the heavy path in Storybook `vitest-ci`.
