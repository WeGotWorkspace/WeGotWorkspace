...
- **`api-quality`** sets `DONE_GATE_SHARD=I/N` on the Composer process. `done-gate.php` reads it: shard 1 still runs greenfield-guard, PHPStan level 1 (committed baseline), and the Architecture suite; every shard runs its slice of unit/feature/storage. Unset locally → full gate, unchanged. Turbo does not run this gate.
